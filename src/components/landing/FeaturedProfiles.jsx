...
export default function FeaturedProfiles() {
  ...
  useEffect(() => {
    const load = async () => {
      const data = await base44.entities.Profile.filter(
        { is_pro: true },
        "-cine_score",
        8
      );
    ...
    <section className="py-24 px-4">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-end justify-between mb-10">
          <div>
            <span className="text-xs uppercase tracking-[0.2em] text-primary font-medium">Featured</span>
            <h2 className="font-display text-3xl sm:text-4xl font-bold text-foreground mt-3">
              Top IMDb Profiles This Week
            </h2>
